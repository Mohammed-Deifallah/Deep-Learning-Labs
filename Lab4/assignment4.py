...
def gru_cell_forward(xt, c_prev, parameters):
    """
        ...
        Note: u/r stand for the update/reset gates, cct stands for the candidate value (c tilde),
              c stands for the memory value
        """
    ...
    ### START CODE HERE ###
    # Compute values for r, u, cct, c_next
    u = sigmoid(np.dot(Wu, np.concatenate((c_prev, xt)) + bu))
    r = sigmoid(np.dot(Wr, np.concatenate((c_prev, xt)) + br))
    cct = np.tanh(np.dot(Wc, np.concatenate((r * c_prev, xt)) + bc))
    c_next = c_prev * (1-u) + u * cct
    # Compute prediction of the GRU cell
    yt_pred = softmax(np.dot(Wy,a_next) + by)
    ### END CODE HERE ###

    # store values needed for backward propagation in cache
    cache = (c_next,  c_prev, u, r, cct, xt, parameters)

    return c_next, yt_pred, cache
  
def gru_forward(x, parameters):
    """
    Implement the forward propagation of the recurrent neural network using an GRU-cell described in Figure (3).
    ...
    # initialize "c" and "y" with zeros
    c = np.zeros((n_a, m, T_x))
    y = np.zeros((n_y, m, T_x))

    # Initialize and c_next
    c_next = np.zeros((n_a, m))
 
    # loop over all time-steps
    for i in range(0, T_x):
        # Update next hidden state, next memory state, compute the prediction, get the cache 
        c_next, yt_pred, cache = lstm_cell_forward(x[:,:,i], c_next, parameters)
        # Save the value of the prediction in y
        y[:,:,i] = yt_pred
        # Save the value of the next cell state
        c[:,:,i] = c_next
